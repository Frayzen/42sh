...
void check_token(struct token *token, struct exp_str *str, enum token_type type)
{
...
struct exp_str *str_create(char *value)
{
    static struct exp_str str;
...
{
    char *str = "\n";
    struct exp_str *value = str_create(str);
